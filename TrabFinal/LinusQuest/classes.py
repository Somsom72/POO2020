...
class Pasta(object):
    ...
    def __new__(cls, nome, change=True, perm=True):
        """
        Método: __new__(cls, nome, change=True, perm=True)
    ...
    def __init__(self, nome, change=True, perm=True):
        """
        Método: __init__(self, nome, change=True, perm=True)
...
class File(object):
    ...
    def __new__(cls, nome, change=True):
        """
        Método: __new__(cls, nome, change=True)
    ...
    def __init__(self, nome, change=True):
        """
        Método: __init__(self, nome, change=True)
...
class Envirorment(object):
    ...
    def __init__(self, user_name):
        """
        Método: __init__(self, user_name)
    ...
    def changeEnv(self):
        ...
        manual.write(
            'Saudações ' + self.user_name + ', sou o Manual do Quartus, um livro falante que por alguma razão\n'+
            'sabe seu nome!\n'+
    ...
    def cmdReader(self, cmd, rep):
        ...
        #grep - encontra uma sequencia de caracteres em um texto
        elif cmd[0] == 'grep':
            #Se o arquivo exixte
            if cmd[-1] in self.pat.files and self.pat.files[cmd[-1]].conteudo is not None:
                #Itera pelas linhas do texto
                for line in self.pat.files[cmd[-1]].conteudo.split('\n'):
                    #Se encontrar a string desejada imprime a linha
                    if cmd[1][1:-1] in line:
                        print('> '+line)
            else:
                print('Arquivo '+cmd[-1]+' vazio ou inexistente')
    ...
    def printLine(self):
        ...
        print(
            "\033[1;32;48m" + self.user_name + '@' + self.user_name + ':' + "\033[1;34;48m" + self.pat.nome + "\033[0;0;0m" + '$',
            end=' ')
    ...
    def segueCond(self, cmd_obj, txt, pat_nome):
        """
        Método: segueCond(self, cmd_obj, txt, pat_nome)
    ...
    def checkFase(self):
        ...
        if (self.fase == 1):
            ...
            self.user_name = name
            print(
                'Linus: Interessante. ' + self.user_name + ', vai também ser útil uma senha simples, digite e nao \nesqueça dela. (Responda e pressione ENTER.)')
        ...
        if (self.fase == 3):
            ...
            if self.exit: return True
            print(
                "Linus: Olha só! Quem precisa de martelos quando temos o terminal?! Agora vamos tentar não morrer \ndo outro lado dessa ponte. Siga-me.")
        ...
        if (self.fase == 5):
            ...
            self.root.filhos['caverna'].filhos['saida'].files['letras.txt'].write(
